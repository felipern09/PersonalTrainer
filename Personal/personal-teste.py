...
referencia = ['1/2023', '2/2023', '3/2023', '4/2023', '5/2023', '6/2023', '7/2023', '8/2023', '9/2023', '10/2023',
              ...
              '11/2025', '12/2025',
              '1/2026', '2/2026', '3/2026', '4/2026', '5/2026', '6/2026', '7/2026', '8/2026', '9/2026', '10/2026',
              '11/2026', '12/2026',
              '1/2027', '2/2027', '3/2027', '4/2027', '5/2027', '6/2027', '7/2027', '8/2027', '9/2027', '10/2027',
              '11/2027', '12/2027',
              '1/2028', '2/2028', '3/2028', '4/2028', '5/2028', '6/2028', '7/2028', '8/2028', '9/2028', '10/2028',
              '11/2028', '12/2028',
              '1/2029', '2/2029', '3/2029', '4/2029', '5/2029', '6/2029', '7/2029', '8/2029', '9/2029', '10/2029',
              '11/2029', '12/2029',
              '1/2030', '2/2030', '3/2030', '4/2030', '5/2030', '6/2030', '7/2030', '8/2030', '9/2030', '10/2030',
              '11/2030', '12/2030',
              '1/2031', '2/2031', '3/2031', '4/2031', '5/2031', '6/2031', '7/2031', '8/2031', '9/2031', '10/2031',
              '11/2031', '12/2031',
              '1/2032', '2/2032', '3/2032', '4/2032', '5/2032', '6/2032', '7/2032', '8/2032', '9/2032', '10/2032',
              '11/2032', '12/2032',
...
def totalhoras():
    ...
    soma = 0
    for aula in aulas:
        soma += aula.simples1 + (aula.simples2 * 2) + aula.dupla1 + (aula.dupla2 * 2) + aula.tripla1 + (
                    aula.tripla2 * 2)
    return soma
...
def gerar_cobranca(caminho):
    ...
    for pessoa in pers:
        ...
        if plan2[(plan2['Serviço'] == 'AULA DE PERSONAL') & (plan2['Duração (horas)'] == 1) & (
                plan2['Cortesia'] == 'N')].empty:
            simples1 = 0
        else:
            simples1 = int(plan2[(plan2['Serviço'] == 'AULA DE PERSONAL') & (plan2['Duração (horas)'] == 1) & (
                        plan2['Cortesia'] == 'N')].iloc[0, 2])

        if plan2[(plan2['Serviço'] == 'AULA DE PERSONAL') & (plan2['Duração (horas)'] == 2) & (
                plan2['Cortesia'] == 'N')].empty:
            simples2 = 0
        else:
            simples2 = int(plan2[(plan2['Serviço'] == 'AULA DE PERSONAL') & (plan2['Duração (horas)'] == 2) & (
                        plan2['Cortesia'] == 'N')].iloc[0, 2])

        if plan2[(plan2['Serviço'] == 'AULA DE PERSONAL 2 PESSOAS') & (plan2['Duração (horas)'] == 1) & (
                plan2['Cortesia'] == 'N')].empty:
            dupla1 = 0
        else:
            dupla1 = int(plan2[(plan2['Serviço'] == 'AULA DE PERSONAL 2 PESSOAS') & (plan2['Duração (horas)'] == 1) & (
                        plan2['Cortesia'] == 'N')].iloc[0, 2])

        if plan2[(plan2['Serviço'] == 'AULA DE PERSONAL 2 PESSOAS') & (plan2['Duração (horas)'] == 2) & (
                plan2['Cortesia'] == 'N')].empty:
            dupla2 = 0
        else:
            dupla2 = int(plan2[(plan2['Serviço'] == 'AULA DE PERSONAL 2 PESSOAS') & (plan2['Duração (horas)'] == 2) & (
                        plan2['Cortesia'] == 'N')].iloc[0, 2])

        if plan2[(plan2['Serviço'] == 'AULA DE PERSONAL 3 PESSOAS') & (plan2['Duração (horas)'] == 1) & (
                plan2['Cortesia'] == 'N')].empty:
            tripla1 = 0
        else:
            tripla1 = int(plan2[(plan2['Serviço'] == 'AULA DE PERSONAL 3 PESSOAS') & (plan2['Duração (horas)'] == 1) & (
                        plan2['Cortesia'] == 'N')].iloc[0, 2])

        if plan2[(plan2['Serviço'] == 'AULA DE PERSONAL 3 PESSOAS') & (plan2['Duração (horas)'] == 2) & (
                plan2['Cortesia'] == 'N')].empty:
            tripla2 = 0
        else:
            tripla2 = int(plan2[(plan2['Serviço'] == 'AULA DE PERSONAL 3 PESSOAS') & (plan2['Duração (horas)'] == 2) & (
                        plan2['Cortesia'] == 'N')].iloc[0, 2])
        ...
        if session.query(Personal).filter_by(nome=str(pessoa)).first() is None:
            result = messagebox.askquestion(f'Personal {str(pessoa).title().split(sep=" ")[0]} é interno?',
                                            f'Personal {str(pessoa).title()} está sendo adicionado no banco de dados do sistema.\nEsse personal é INTERNO?')
            if result == 'yes':
                adicionar = Personal(nome=str(pessoa), email='marcelo.goncalves@ciaathletica.com.br',
                                     tipo_personal='Interno', status='Ativo')
                session.add(adicionar)
                session.commit()
            else:
                adicionar = Personal(nome=str(pessoa), email='marcelo.goncalves@ciaathletica.com.br',
                                     tipo_personal='Externo', status='Ativo')
                session.add(adicionar)
                session.commit()
            ...
            mesp = session.query(Aulas).filter_by(mes=f'{mespassado.month}/{mespassado.year}').filter_by(
                personal=prof.id).first()
            if mesp:
            ...
            jaexiste = session.query(Aulas).filter_by(mes=f'{hoje.month}/{hoje.year}').filter_by(
                personal=prof.id).first()
            if jaexiste:
...
def gerarecibo(nome):
    if nome:
        ...
        if usuario:
            ...
            recibo.paragraphs[11].text = str(recibo.paragraphs[11].text).replace('#valor#', '{:,.2f}'.format(
                aula.valorpago).replace('.', ','))
            recibo.paragraphs[11].text = str(recibo.paragraphs[11].text).replace('#extens', extenso)
            recibo.paragraphs[11].text = str(recibo.paragraphs[11].text).replace('#indiv#',
                                                                                 str(aula.simples1 + aula.simples2).replace(
                                                                                     '.', ','))
            recibo.paragraphs[11].text = str(recibo.paragraphs[11].text).replace('#dupl#',
                                                                                 str(aula.dupla1 + aula.dupla2).replace(
                                                                                     '.', ','))
            recibo.paragraphs[11].text = str(recibo.paragraphs[11].text).replace('#tripl#',
                                                                                 str(aula.tripla1 + aula.tripla2).replace(
                                                                                     '.', ','))
            ...
            recibo.paragraphs[13].text = str(recibo.paragraphs[13].text).replace('#valorcr#',
                                                                                 '{:,.2f}'.format(aula.credito).replace(
                                                                                     '.', ','))
            recibo.paragraphs[14].text = str(recibo.paragraphs[14].text).replace('#valordeb#',
                                                                                 '{:,.2f}'.format(aula.debito).replace(
                                                                                     '.', ','))
            recibo.save(f'.\\Relatorios\\{hoje.year}\\{str(hoje.month).zfill(2)}\\Recibos\\Recibo {nomerec}.docx')
            t.sleep(1)
            entrada = os.path.abspath(
                f'.\\Relatorios\\{hoje.year}\\{str(hoje.month).zfill(2)}\\Recibos\\Recibo {nomerec}.docx')
            saida = os.path.abspath(
                f'.\\Relatorios\\{hoje.year}\\{str(hoje.month).zfill(2)}\\Recibos\\Recibo {nomerec}.pdf')
            ...
            part.add_header('Content-Disposition', 'attachment',
                            filename=f'Recibo personal {nomerec} {hoje.month}-{hoje.year}.pdf')
            msg.attach(part)
            s.send_message(msg)
            del msg
            s.quit()
...
def salvarrelatoriomes(mes):
    ...
    plan = l_w(os.path.abspath(
        f'.\\Relatorios\\{anoref}\\{str(mesref).zfill(2)}\\Controle\\Relatório Personal {mesref}-{anoref}.xlsx'),
               read_only=False)
    aba = plan['Sheet']
...
def reajuste(valor):
    ...
    valoraulas.internosimplesnodesc = round(valoraulas.internosimplesnodesc + (valoraulas.internosimplesnodesc * num),
                                            2)
    ...
    valoraulas.externosimplesnodesc = round(valoraulas.externosimplesnodesc + (valoraulas.externosimplesnodesc * num),
                                            2)
    ...
    valoraulas.internosimples51a100 = round(valoraulas.internosimples51a100 + (valoraulas.internosimples51a100 * num),
                                            2)
    valoraulas.internosimples101a120 = round(
        valoraulas.internosimples101a120 + (valoraulas.internosimples101a120 * num), 2)
    valoraulas.internosimplesacima120 = round(
        valoraulas.internosimplesacima120 + (valoraulas.internosimplesacima120 * num), 2)
    ...
    valoraulas.internoduplaacima119 = round(valoraulas.internoduplaacima119 + (valoraulas.internoduplaacima119 * num),
                                            2)
    ...
    valoraulas.internotriplaacima119 = round(
        valoraulas.internotriplaacima119 + (valoraulas.internotriplaacima119 * num), 2)
    ...
    valoraulas.externosimples51a100 = round(valoraulas.externosimples51a100 + (valoraulas.externosimples51a100 * num),
                                            2)
    valoraulas.externosimples101a120 = round(
        valoraulas.externosimples101a120 + (valoraulas.externosimples101a120 * num), 2)
    valoraulas.externosimplesacima120 = round(
        valoraulas.externosimplesacima120 + (valoraulas.externosimplesacima120 * num), 2)
    ...
    valoraulas.externoduplaacima119 = round(valoraulas.externoduplaacima119 + (valoraulas.externoduplaacima119 * num),
                                            2)
    ...
    valoraulas.externotriplaacima119 = round(
        valoraulas.externotriplaacima119 + (valoraulas.externotriplaacima119 * num), 2)
    session.commit()
...
def janelacontato():
    ...
    ttk.Button(j1, text="Solicitar Suporte", command=lambda: [suporte(assunto.get(), pegartexto()),
                                                              tkinter.messagebox.showinfo(title='Pedido ok!',
                                                                                          message='Pedido de suporte enviado com sucesso!'),
                                                              j1.destroy()]).grid(column=1, row=6, padx=40, pady=10,
                                                                                  sticky=E)
...
def porcent():
    ...
    ttk.Button(jj2, text='Lançar reajuste', command=lambda: [reajuste(porc.get()),
                                                             tkinter.messagebox.showinfo(title='Reajuste feito!',
                                                                                         message='Reajuste de horas lançado com sucesso!'),
                                                             jj2.destroy()]).grid(column=1, row=4, sticky=E, pady=5,
                                                                                  padx=15)
...
def janelavalores():
    ...
    ttk.Label(j2, text=f"Simples sem desconto: {valores.internoduplanodesc}").grid(column=1, row=2, sticky=W, pady=10,
                                                                                   padx=15)
    ttk.Label(j2, text=f"Simples (1 a 10 aulas): {valores.internosimples1a10}").grid(column=1, row=3, sticky=W, padx=15)
    ttk.Label(j2, text=f"Simples (11 a 30 aulas): {valores.internosimples11a30}").grid(column=1, row=4, sticky=W,
                                                                                       padx=15)
    ttk.Label(j2, text=f"Simples (31 a 50 aulas): {valores.internosimples31a50}").grid(column=1, row=5, sticky=W,
                                                                                       padx=15)
    ttk.Label(j2, text=f"Simples (51 a 100 aulas): {valores.internosimples51a100}").grid(column=1, row=6, sticky=W,
                                                                                         padx=15)
    ttk.Label(j2, text=f"Simples (101 a 120 aulas): {valores.internosimples101a120}").grid(column=1, row=7, sticky=W,
                                                                                           padx=15)
    ttk.Label(j2, text=f"Simples (acima de 120 aulas): {valores.internosimplesacima120}").grid(column=1, row=8,
                                                                                               sticky=W, padx=15)
    ttk.Label(j2, text=f"Dupla sem desconto: {valores.internoduplanodesc}").grid(column=1, row=9, sticky=W, pady=10,
                                                                                 padx=15)
    ttk.Label(j2, text=f"Dupla (1 a 60 aulas): {valores.internodupla1a60}").grid(column=1, row=10, sticky=W, padx=15)
    ttk.Label(j2, text=f"Dupla (61 a 119 aulas): {valores.internodupla61a119}").grid(column=1, row=11, sticky=W,
                                                                                     padx=15)
    ttk.Label(j2, text=f"Dupla (acima de 119 aulas): {valores.internoduplaacima119}").grid(column=1, row=12, sticky=W,
                                                                                           padx=15)
    ttk.Label(j2, text=f"Tripla sem desconto: {valores.internotriplanodesc}").grid(column=1, row=13, sticky=W, pady=10,
                                                                                   padx=15)
    ttk.Label(j2, text=f"Tripla (1 a 60 aulas): {valores.internotripla1a60}").grid(column=1, row=14, sticky=W, padx=15)
    ttk.Label(j2, text=f"Tripla (61 a 119 aulas): {valores.internotripla61a119}").grid(column=1, row=15, sticky=W,
                                                                                       padx=15)
    ttk.Label(j2, text=f"Tripla (acima de 119 aulas): {valores.internotriplaacima119}").grid(column=1, row=16, sticky=W,
                                                                                             padx=15)
    ttk.Label(j2, text=f"Valores para personais Externos:").grid(column=2, row=1, sticky=W, pady=5, padx=15)
    ttk.Label(j2, text=f"Simples sem desconto: {valores.externosimplesnodesc}").grid(column=2, row=2, sticky=W, pady=10,
                                                                                     padx=15)
    ttk.Label(j2, text=f"Simples (1 a 10 aulas): {valores.externosimples1a10}").grid(column=2, row=3, sticky=W, padx=15)
    ttk.Label(j2, text=f"Simples (11 a 30 aulas): {valores.externosimples11a30}").grid(column=2, row=4, sticky=W,
                                                                                       padx=15)
    ttk.Label(j2, text=f"Simples (31 a 50 aulas): {valores.externosimples31a50}").grid(column=2, row=5, sticky=W,
                                                                                       padx=15)
    ttk.Label(j2, text=f"Simples (51 a 100 aulas): {valores.externosimples51a100}").grid(column=2, row=6, sticky=W,
                                                                                         padx=15)
    ttk.Label(j2, text=f"Simples (101 a 120 aulas): {valores.externosimples101a120}").grid(column=2, row=7, sticky=W,
                                                                                           padx=15)
    ttk.Label(j2, text=f"Simples (acima de 120 aulas): {valores.externosimplesacima120}").grid(column=2, row=8,
                                                                                               sticky=W, padx=15)
    ttk.Label(j2, text=f"Dupla sem desconto: {valores.externoduplanodesc}").grid(column=2, row=9, sticky=W, pady=10,
                                                                                 padx=15)
    ttk.Label(j2, text=f"Dupla (1 a 60 aulas): {valores.externodupla1a60}").grid(column=2, row=10, sticky=W, padx=15)
    ttk.Label(j2, text=f"Dupla (61 a 119 aulas): {valores.externodupla61a119}").grid(column=2, row=11, sticky=W,
                                                                                     padx=15)
    ttk.Label(j2, text=f"Dupla (acima de 119 aulas): {valores.externoduplaacima119}").grid(column=2, row=12, sticky=W,
                                                                                           padx=15)
    ttk.Label(j2, text=f"Tripla sem desconto: {valores.externotriplanodesc}").grid(column=2, row=13, sticky=W, pady=10,
                                                                                   padx=15)
    ttk.Label(j2, text=f"Tripla (1 a 60 aulas): {valores.externotripla1a60}").grid(column=2, row=14, sticky=W, padx=15)
    ttk.Label(j2, text=f"Tripla (61 a 119 aulas): {valores.externotripla61a119}").grid(column=2, row=15, sticky=W,
                                                                                       padx=15)
    ttk.Label(j2, text=f"Tripla (acima de 119 aulas): {valores.externotriplaacima119}").grid(column=2, row=16, sticky=W,
                                                                                             padx=15)
...
def janelarelatorio():
    ...
    def mesderef(event):
        ...
        if x:
            ...
            trec.config(
                text='Total recebido até o momento: R$ {:,.2f}'.format(round(totalpago, 2)).replace(',', '_').replace(
                    '.', ',').replace('_', '.'))
            tarec.config(text='Total a receber: R$ {:,.2f}'.format(round(totaldevido, 2)).replace(',', '_').replace('.',
                                                                                                                    ',').replace(
                '_', '.'))
        else:
    ...
    ttk.Button(j3, text="Fechar Mês", command=lambda: [fecharmes(refmes.get()),
                                                       tkinter.messagebox.showinfo(title=f'Mês {refmes.get()} Fechado!',
                                                                                   message='Valores não pagos lançados como débito!')]).grid(
        column=2, row=7, padx=40, pady=15, sticky=W)
    ttk.Button(j3, text="Salvar relatório em excel", command=lambda: [salvarrelatoriomes(refmes.get()),
                                                                      tkinter.messagebox.showinfo(
                                                                          title='Relatório Salvo!',
                                                                          message='Relatório salvo com sucesso!\n\nRelatório salvo em Personal > Relatórios > "ano" > "mês" > Controle.')]).grid(
        column=2, row=8, padx=40, pady=15, sticky=W)
...
def janeladadospers():
    ...
    ttk.Button(j4, text="Salvar dados",
               command=lambda: [alterarpersonal(quem.get(), texttipo.get(), textwpp.get(), textemail.get()),
                                tkinter.messagebox.showinfo(
                                    title='Dados Salvos!',
...
ttk.Button(inicial, text="Cadastrar",
           command=lambda: [cadastrarpersonal(entrada_nome.get(), entrada_email.get(), entrada_tel.get(), radio.get()),
                            entrada_nome.delete(0, END), entrada_email.delete(0, END), entrada_tel.delete(0, END),
                            tkinter.messagebox.showinfo(title='Cadastro Ok!',
                                                        message='Cadastro efetuado com sucesso!')]).grid(column=2,
                                                                                                         row=16,
                                                                                                         sticky=E)
...
ttk.Button(calcular, text="Calcular valores das aulas", command=lambda: [gerar_cobranca(caminho.get()), mostrar(),
                                                                         tkinter.messagebox.showinfo(
                                                                             title='Cálculo ok!',
                                                                             message='Cálculo de horas efetuado com sucesso!')]).grid(
    column=2, row=22, padx=20, pady=10, sticky=E)
if session.query(Aulas).filter_by(mes=f'{hoje.month}/{hoje.year}').first():
    ttk.Label(calcular, text=f'Personal mês: {hoje.month}/{hoje.year}.').grid(column=1, row=27, padx=28, pady=8,
                                                                              sticky=W)
    ...
    ttk.Label(calcular, text=f'Total aproximado de receita: {totalreceita()}.').grid(column=1, row=31, padx=28,
                                                                                     sticky=W)
...
def ver(event):
    ...
    if x:
        ...
        if aula:
            ...
            label13.config(
                text='Total de taxa com desconto: R$ {:,.2f}'.format(aula.valortotalemdia).replace(',', '_').replace(
                    '.', ',').replace('_', '.'))
            label14.config(
                text='Total de taxa sem desconto: R$ {:,.2f}'.format(aula.valortotalatraso).replace(',', '_').replace(
                    '.', ',').replace('_', '.'))
        else:
...
ttk.Button(verificar, text="Lançar ajuste", command=lambda: [
    ajuste(combo.get(), comboaulad.get(), aulas.get(), desconto.get(), comboaulaa.get(), aulasacr.get(),
           acrescimo.get()), tkinter.messagebox.showinfo(title='Lançamento ok!', message='Ajuste lançado com sucesso!'),
    desconto.set(''), acrescimo.set(''), aulas.set(''), aulasacr.set(''), descaula.delete(0, END), desc.delete(0, END),
    comboaulad.set(''), comboaulaa.set(''), acresaula.delete(0, END), acres.delete(0, END)]).grid(column=3, row=13,
                                                                                                  sticky=E)
ttk.Button(verificar, text="Enviar todos e-mails", command=lambda: [mandar_email(),
                                                                    tkinter.messagebox.showinfo(title='Sucesso!',
                                                                                                message='Operação efetuada com sucesso!')]).grid(
    column=1, row=20, padx=20, sticky=W, pady=20)
...
ttk.Button(pagamentos, text="Lançar pagamento",
           command=lambda: [pagamento(combo_nome.get(), pgto.get(), valorpag.get()),
                            tkinter.messagebox.showinfo(title='Lançamento ok!',
                                                        message='Pagamento lançado com sucesso!'),
                            entrada_pgto.delete(0, END)]).grid(column=2, row=22, sticky=W)
...
def valoresrecibo(event):
    ...
    if session.query(Personal).filter_by(nome=nome).first():
        ...
        if aula:
            ...
            rec4.config(
                text='Valor cobrado: R$ {:,.2f}'.format(aula.valorcobrado).replace(',', '_').replace('.', ',').replace(
                    '_', '.'))
            rec6.config(
                text='Valor pago: R$ {:,.2f}'.format(aula.valorpago).replace(',', '_').replace('.', ',').replace('_',
                                                                                                                 '.'))
            rec7.config(
                text='Crédito: R$ {:,.2f}'.format(aula.credito).replace(',', '_').replace('.', ',').replace('_', '.'))
            rec8.config(
                text='Débito: R$ {:,.2f}'.format(aula.debito).replace(',', '_').replace('.', ',').replace('_', '.'))
        else:
...
ttk.Button(recibos, text="Enviar recibo por e-mail", command=lambda: [gerarecibo(comborecibo.get()),
                                                                      tkinter.messagebox.showinfo(title='Recibo ok!',
                                                                                                  message='Recibo enviado com sucesso!')]).grid(
    column=2, row=24, padx=20, pady=20, sticky=W)
...
def valoresinad(event):
    ...
    if session.query(Personal).filter_by(nome=nome).first():
        ...
        if aula:
            ...
            inad4.config(text='Valor devido: R$ {:,.2f}'.format(aula.valortotalatraso).replace(',', '_').replace('.',
                                                                                                                 ',').replace(
                '_', '.'))
        else:
...
ttk.Button(inadimplentes, text="Enviar cobrança para todos", command=lambda: [cobrar(), tkinter.messagebox.showinfo(
    title='Cobrança ok!', message='Cobrança enviada com sucesso!')]).grid(column=2, row=24, padx=20, pady=20, sticky=W)
